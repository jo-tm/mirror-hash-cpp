...
class Mirror256 {
public:
    Mirror256(const std::string &m = "", uint32_t depth = DEFAULT_DEPTH, uint32_t size = DEFAULT_SIZE, bool useStandardState = true);
    void update(const std::string &m);
    std::string digest();
    std::string hexdigest();

private:
    std::vector<uint8_t> unpack(const std::string &m);
    std::string pack(const std::vector<uint8_t> &hm);
    void initStandardState();
    void initLastHashes();
    std::vector<uint8_t> randomHash();
    std::vector<uint8_t> _mirror256_process(const std::string &m);
    std::vector<uint8_t> hashLayerPass(uint32_t layer, const std::vector<uint8_t> &block);
    uint32_t getWire(uint32_t gateIndex, bool firstSublayer, uint32_t offset = 0);
    uint8_t getBit(const std::vector<uint8_t> &block, uint32_t wire);
    void setBit(std::vector<uint8_t> &block, uint32_t wire, uint8_t bit);
    std::vector<uint8_t> applyGate(uint32_t gateIndex, uint32_t gateName, uint32_t gateSymmetry, const std::vector<uint8_t> &block, bool firstSublayer);

    std::string _buffer;
    uint64_t _counter;
    uint32_t depth = DEFAULT_DEPTH;
    std::vector<std::vector<uint8_t>> lastHashes;
    std::vector<uint8_t> _hashed;

    static constexpr uint32_t DEFAULT_DEPTH = 256;
    static constexpr uint32_t DEFAULT_SIZE = 256;
    static constexpr uint8_t GATES[2] = {0, 1};
    static constexpr uint64_t firstPrimesCubicRootDecRep[512] = {
